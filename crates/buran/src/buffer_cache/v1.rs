...
//! ### Todos
//!
//! - [ ] Asynchronous faults and flushes. Each frame should store a signal that can
//! be waited on by multiple concurrent threads while a background thread performs
//! the I/O. OnceCell, but resetable and (preferably) suppoorting both async and
//! blocking.
...
impl BufferCache {
    ...
    fn fetch_page(&self, i: NonZeroU64) -> Result<(usize, RwLockWriteGuard<'_, ()>)> {
        // Concurrency note: Multiple threads may attempt to fault the same page
        // simultaneously. We use the frame mapping table as a concurrency control
        // mechanism by grabbing the entry before performing the fault. Then,
        // we check if a frame has already been assigned. If so, then another thread
        // has performed the fault and we can fall back to this.

        let entry = match self.frame_mappings.entry(i) {
            dashmap::mapref::entry::Entry::Occupied(e) => {
                let index = *e.get();
                let guard = self.frames[index].lock.write();
    ...
    fn record_access(&self, index: usize) {
        self.cache_state.lock().access(&self.frames, index);
    }
}
...
pub struct CacheConfig {
    ...
    pub cache_access_buffer_capacity: usize,
}
...
impl CacheState {
    pub fn access(&mut self, frames: &[Frame], frame: usize) {
        let get = |i: usize| frames[i].page_number().get();

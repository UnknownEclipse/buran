...
pub struct WTinyLfu {
    ...
    /// this!!)
    freq_sketch: Sketch<NonZeroU64, ahash::RandomState>,
    freq_counter: usize,
}

impl WTinyLfu {
    pub fn insert(&mut self, index: usize, get_pgno: impl Fn(usize) -> NonZeroU64) {
    ...
    pub fn access(&mut self, index: usize, get_pgno: impl Fn(usize) -> NonZeroU64) {
        self.inc_freq(get_pgno(index));
    ...
    fn promote_if_worthy(&mut self, index: usize, get_pgno: impl Fn(usize) -> NonZeroU64) -> bool {
    ...
    fn inc_freq(&mut self, pgno: NonZeroU64) {
...
impl TinyLfuBuilder {
    ...
    pub(super) fn build(&self) -> WTinyLfu {
        let window_size = self.capacity as f32 * self.window_fraction;
        let main_size = self.capacity - window_size as usize;

        let probation_size = main_size / 5;
        let protected_size = main_size - probation_size;

        let width = usize::BITS as usize / 2;
        let region_words = (self.capacity + width - 1) / width;

        let sample_size = self.capacity * self.sample_size_coefficient;
        ...
        WTinyLfu {
            ...
            links: vec![Default::default(); self.capacity].into_boxed_slice(),
...
impl Default for TinyLfuBuilder {
    fn default() -> Self {
        Self {
            capacity: 0,
            window_fraction: 0.1,
